...
class MP4Remuxer {
  ...
  remuxAudio(track,timeOffset, contiguous, t0) {
      var view,
          offset = 8,
          pesTimeScale = this.PES_TIMESCALE,
      ...
      if ( contiguous && this.nextAacPts) { t0 = this.nextAacPts/pesTimeScale; }
      ...
      if (nSamples) {
          let endPTS = (samples0[nSamples -1].pts - samples0[0].pts) + firstPTS;
          mp4SampleDuration = (endPTS - firstPTS) / ( (nSamples - 1) * pes2mp4ScaleFactor );

          let endDTS = (samples0[nSamples -1].dts - samples0[0].dts) + firstPTS;
          let mp4SampleDuration_2 = (endDTS - firstDTS) / ( (nSamples - 1) * pes2mp4ScaleFactor );
      ...
      if (nbSamples) {
          ...
          moof = MP4.moof(track.sequenceNumber++, t0*track.audiosamplerate, track);
